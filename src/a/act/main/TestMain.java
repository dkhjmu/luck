...
public class TestMain {
	...
	public static void main(String[] args) {
		...
		ArrayList<CalcVO> gapResult = gap.calc(list, 311, seq);
		Gap2Calc gap2=new Gap2Calc();
		ArrayList<CalcVO> gap2Result = gap2.calc(list, 311, seq, 2);
		...
		CountCalc cal=new CountCalc();
		ArrayList<CalcVO> aa = cal.calc(list, seq-42+1, seq);
		
		String[] rr=new String[45];
		for(int i=0;i<45;i++){
			rr[i]=(i+1)+"\t"
						+gapResult.get(i).get(GapCalc.GAP)+"\t"
						+gap2Result.get(i).get(Gap2Calc.GAP)+"\t"
						+aa.get(i).get(CountCalc.CNT)+"\t";
			for(int j=0;j<bigResult.size();j++){
				CalcVO temp = bigResult.get(j).get(i);
				rr[i]=rr[i]+temp.get(CountCalc.CNT)+"\t";
			}
		}
